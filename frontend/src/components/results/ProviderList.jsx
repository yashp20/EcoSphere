...
export default function ProviderList({ zip }) {
  ...
  useEffect(() => {
    async function loadProviders() {
      try {
        const res = await axios.get(`http://localhost:8000/api/providers/${zip}`);
  ...
  return (
    <div className="space-y-4">
      {providers.map((p, idx) => (
        <div
          key={idx}
          className="bg-white p-6 rounded-2xl shadow border border-green-200 cursor-pointer transition hover:shadow-lg"
